...
class LibraryDAOService: LibraryDAO {
    ...
    override fun findAllByAuthor(author: String): Set<Book> {
        return this.books.filter { it.author?.contains(author, ignoreCase = true) == true }.toSet()
    }

    override fun findAllByTitle(title: String): Set<Book> {
        return this.books.filter { it.name?.contains(title, ignoreCase = true) == true }.toSet()
    }

    override fun findAllByPublisher(publisher: String): Set<Book> {
        return this.books.filter { it.publisher?.contains(publisher, ignoreCase = true) == true }.toSet()
    }
}